...
Notation "x + y" := fp[[x, y]].
Notation "0" := zeroR.
Notation "x · y" := fm[[x, y]](at level 40).
Notation "1" := oneR.
Notation "x ≤ y" := ([x, y] ∈ Leq)(at level 77).
Notation "- a" := (∩(\{ λ u, u ∈ R /\ u + a = 0 \})).
Notation "x - y" := (x + (-y)).
Notation "a ⁻" := (∩(\{ λ u, u ∈ (R ~ [0]) /\ u · a = 1 \}))(at level 5).
Notation "m / n" := (m · (n⁻)).
...
Reserved Notation "1".
Reserved Notation "x ≤ y"(at level 77).
Notation "x ≤ y" := ([x, y] ∈ Leq).
Reserved Notation "- a" (at level 35, right associativity).
Reserved Notation "x - y"(at level 50, left associativity).
Reserved Notation "a ⁻"(at level 0).
Reserved Notation "m / n" (at level 40, left associativity).
...
Class Real_axioms (RR : Real_struct):= {
  ...
  Leq_P3 : ∀ x y z, x ∈ R -> y ∈ R -> z ∈ R -> x ≤ y -> y ≤ z
   -> x ≤ z;
  Leq_P4 : ∀ x y, x ∈ R -> y ∈ R -> x ≤ y \/ y ≤ x;
  Plus_Leq : ∀ x y z, x ∈ R -> y ∈ R -> z ∈ R -> x ≤ y
   -> x + z ≤ y + z;
  Mult_Leq : ∀ x y, x ∈ R -> y ∈ R -> 0 ≤ x -> 0 ≤ y
   -> 0 ≤ x · y;
...
Hint Resolve PlusR : real.
Hint Resolve zero_in_R : real.
Hint Resolve MultR : real.
Hint Resolve one_in_R : real.
...
Hint Resolve one_in_R one_in_R_Co : real.
...
Hint Resolve Plus_neg1a Plus_neg1b Plus_neg2 Minus_P1
  Minus_P2 : real.
...
Hint Resolve Mult_inv1 Mult_inv2 Divide_P1 Divide_P2 : real.
...
Corollary Max_Corollary : ∀ X c1 c2, Max X c1 -> Max X c2
  -> c1 = c2.
Proof.
...
Corollary Min_Corollary : ∀ X c1 c2, Min X c1 -> Min X c2
  -> c1 = c2.
Proof.
  intros. destruct H as [H[]],H0 as [H0[]].
  pose proof H1. pose proof H3. apply H2 in H5.
...
Hint Resolve N_Subset_R one_in_N : real.
...
(* 自然数的性质 *)
Proposition Nat_P1a : ∀ m n, m ∈ N -> n ∈ N
  -> (m + n) ∈ N.
Proof.
...
Proposition Nat_P1b : ∀ m n, m ∈ N -> n ∈ N
  -> (m · n) ∈ N.
Proof.
  intros. set (E := \{ λ u, u ∈ N /\ ∀ v, v ∈ N -> (u · v) ∈ N \}).
  assert (1 ∈ E).
...
Hint Resolve Nat_P1a Nat_P1b : real.
...
Hint Resolve N_Subset_Z Z_Subset_R Int_P1a Int_P1b : real.
...
Hint Resolve Z_Subset_Q Q_Subset_R Rat_P1a Rat_P1b : real.
...
Lemma Even_and_Odd_P2_Lemma : ∀ m n, m ∈ Z -> n ∈ Z -> n < m
  -> (n + 1) ≤ m.
Proof.
...
Proposition Even_and_Odd_P3 : ∀ r, r ∈ N -> Even (r · r)
  -> Even r.
Proof.
...
Lemma Existence_of_irRational_Number_Lemma1 :
  ∀ a b, a ∈ R -> b ∈ R -> a <> b
  -> (((b · b) - (a · a)) < ((1 + 1) · b · (b - a))).
Proof.
...
Lemma Existence_of_irRational_Number_Lemma2 : ∀ x y,
  x ∈ R -> y ∈ R -> x < y -> ∃ r, r ∈ R /\ x < r /\ r < y.
Proof.
  intros. assert (0 < 1 + 1).
  { apply (Order_Co2 _ 1); auto with real. left.
...
Lemma Existence_of_irRational_Number_Lemma3 : ∀ x y,
  x ∈ R -> y ∈ R -> 0 < y -> (y · y) < x
  -> ∃ r, r ∈ R /\ y < r /\ 0 < r /\ (r · r) < x.
Proof.
...
Lemma Existence_of_irRational_Number_Lemma4 : ∀ x y,
  x ∈ R -> y ∈ R -> 0 < y -> 0 < x -> x < (y · y)
  -> ∃ r, r ∈ R /\ r < y /\ 0 < r /\ x < (r · r).
Proof.
  intros. assert (0 < 1 + 1).
  { apply (Order_Co2 _ 1); auto with real. right.
...
Lemma Existence_of_irRational_Number_Lemma5 : ∀ r, r ∈ R
  -> 0 < r -> (∃ x, x ∈ R /\ 0 < x /\ x · x = r).
Proof.
...
Lemma Existence_of_irRational_Number_Lemma6 : ∀ r, r ∈ Q
  -> 0 < r -> ∃ a b, a ∈ N /\ b ∈ N /\ r = a / b.
Proof.
...
Lemma Arch_P3_Lemma : ∀ m n, m ∈ Z -> n ∈ Z -> n < m
  -> (n + 1) ≤ m.
Proof.
  intros. pose proof H. apply Z_Subset_R,(Order_Co1 m (n + 1))
  in H2 as [H2|[]]; auto with real;
...
Notation "］ a , b ［" := (\{ λ x, x ∈ R /\ a < x /\ x < b \})
  (at level 5, a at level 0, b at level 0).
...
Notation "］ a , +∞［" := (\{ λ x, x ∈ R /\ a < x \})
  (at level 5, a at level 0).

Notation "［ a , +∞［" := (\{ λ x, x ∈ R /\ a ≤ x \})
  (at level 5, a at level 0).

Notation "］-∞ , b ］" := (\{ λ x, x ∈ R /\ x ≤ b \})
  (at level 5, b at level 0).

Notation "］-∞ , b ［" := (\{ λ x, x ∈ R /\ x < b \})
  (at level 5, b at level 0).

Notation "]-∞ , +∞[" := (R)(at level 0).
